# Investigation Timeline — PowerShell Remoting Investigation

## Timeline Overview

This timeline separates **investigation actions** from **observed endpoint telemetry**.

Commands executed by the analyst are not automatically treated as historical attacker activity.
...
The expected remote execution sequence is:
...
The timeline demonstrates that the endpoint had WinRM configuration, an enabled HTTP listener, WSMan Operational activity, and PowerShell process creation.

However, the available evidence does not establish the complete remote execution sequence.
...
WinRM configuration
+
Local WSMan activity
+
PowerShell process activity
```

but does not support a confirmed claim of:
...
> **An event belongs to the investigation timeline, but its meaning depends on correlation and context.**
